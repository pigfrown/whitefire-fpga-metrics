...
class FXTokenMinerParser():
    """
    Parses FXTokenMiner 0xBTC stream.
    ...
    def __init__(self, miner_output, cardname, logging=True):
        ...
        #Setup logging
        if self.logging:
            logname = "FXTokenParser-{}".format(self.name)
...
class BitstreamLogExporter():
    """
    Reads miner out from a file and uses FXTokenMinerParser to parse the output"
    ...
    def start(self):
        ...
        for logpath, name in zip(self.log_paths, self.card_names):
            ...
            parsers.append(FXTokenMinerParser(log_handle, name, logging=self.logging))
